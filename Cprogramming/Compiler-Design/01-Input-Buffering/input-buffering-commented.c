...
int main()
{
	...
	while(!(memory[indexRAM] == '\0'))//memory to buffer until memory reach to \0
	{
		for(i = 0; i < N; i++, fp++)
		{
			if(turn == 1 && bufferEnd(buffer1))//buffer1 size is 5 and calling util fun
			{
				fillBuffer(buffer2);//another util function
				turn = 2;
				i = 0;
			}
...
int bufferEnd(char buffer[N])
{
	if(buffer[fp % N] == '\0' && memory[indexRAM] != '\0')//memory not empty and buffer forward pointer should not be enf
		return 1; 
	else return 0;
}
int lexemeEnd(char buffer[N])
{
	if((buffer[fp % N] == ' ' || buffer[fp % N] == ';'))//if  buffer contain space or ; , return 1 else 0 
		return 1;
	else return 0;
}
void fillBuffer(char buffer[N])
{
	int i;
	strcpy(buffer, "");		// Clear buffer, buffer
